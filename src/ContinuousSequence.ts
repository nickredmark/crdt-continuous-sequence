export class ContinuousSequence {
    ...
    move = (current: any, prev: any, next: any) => this.setIndex(current, this.getIndexBetween(current, prev, next))
    ...
    getIndexBetween = (current: any, prev: any, next: any) => {
        ...
        const currentIndex = this.getIndex(current);
        ...
        if (nextIndex) {
            ...
            while (this.compareIndexPart(prevIndex[index.length], nextIndex[index.length]) === 0) {
                index.push(prevIndex[index.length]);
            }
            // make sure we are before next
            if (this.compareIndexPart(nextIndex[index.length], currentIndex) <= 0) {
                index.push(prevIndex[index.length]);
            }
        }
        // make sure we are before prev
        while (this.compareIndexPart(currentIndex, prevIndex[index.length]) <= 0) {
            index.push(prevIndex[index.length]);
        }
    ...
    getId = (element: any) => typeof element === 'string' ? element : element.id
